    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (inputs, targets) in enumerate(trainloader):
            ...
            if hard_num > 0:
                ...
                loss3 = nn.MarginRankingLoss(margin=dist)(gt_scores.view(-1, 1), max_novel_scores.view(-1, 1), torch.ones(hard_num*K, 1).to(device)) * lw_mr
            else:
                loss3 = torch.zeros(1).to(device)

            # Loss 4: logits-level distillation loss
